...
phylo.betapart.core.w<-function(x, tree)
{
  if (!is.matrix(x)) {
    x <- as.matrix(x)
  }
...
#### end of New Code Chunk ####
  ...
  if(canRunInParallel) {
    ...
    logFile <- paste(getwd(),"/temp_file_betapart",gsub("[^[:digit:]]", "", Sys.time()),".txt",sep="")
    file.create(logFile)
  ...
  pd.tot.pair.list <- pblapply(chunks, function(chunk) {   # PD of the two communities combined
    ...
    PhyloMeasures::pd.query(tree,com.tot.pair,standardize = T)
  }, cl = cores,mc.cleanup = T)
  pd.tot.pair <- unlist(pd.tot.pair.list)
  # if(canRunInParallel) {
  #   stopCluster(cl)
  # }
  print('computing sum of PD for each community, separetely')
  # sum.pd.pair <- pbapply(combin,2,function(x) sum(pd[x]))# Sum of PD for each community, separetely
  ...
  chunks2=chunks
  min.not.shared.par <- unlist(pblapply(chunks2, function(chunk) {   # minimun (b,c) with paralization
    chunkId <- chunk$chunkId
    chunk$data[]<- pd[chunk$data]
    apply(pd.tot.pair.list[[chunkId]]-t(chunk$data),1,min)
  }, cl = cores,mc.cleanup = T))
  
  #restart the object to run the maximum
  chunks2=chunks
  print("calculating maximum not shared PD")
  max.not.shared.par <- unlist(pblapply(chunks2, function(chunk) {   # minimun (b,c) with paralization
    chunkId <- chunk$chunkId
    chunk$data[]<- pd[chunk$data]
    apply(pd.tot.pair.list[[chunkId]]-t(chunk$data),1,max)
  }, cl = cores,mc.cleanup = T))
...
phylo.beta.pair.stand<- function (x, tree, index.family = "sorensen") 
{
